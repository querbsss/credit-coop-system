      ...
      <main className="loan-application-main">
        <div className="container">
          <div className="page-header">
            <h1>📍 Loan Tracker</h1>
            <p>Track the status of your loan application and see calculated amounts when a loan amount is assigned.</p>
          </div>

          {loading && (
            <div className="card">
              <p>Loading your loan application...</p>
            </div>
          )}

          {!loading && error && (
            <div className="card status-message error">❌ {error}</div>
          )}

          {!loading && !application && !error && (
            <div className="card">
              <h3>No loan application found</h3>
              <p>You do not have any submitted loan applications. If you'd like to apply, go to the Loan Application page.</p>
          ...
          {!loading && application && (
            <div className="card">
              ...
              <p><strong>Status:</strong> {application.status ?? application.review_status ?? 'N/A'}</p>
              ...
              {(application.loan_amount || application.amount || application.requested_amount) ? (
                ...
                <div style={{ marginTop: '1rem' }}>
                  <p>No loan amount has been assigned yet. Please check back later or contact support.</p>
                </div>
              )}
